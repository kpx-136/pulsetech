...
function appendProductCard(product) {
	...
	productCard.classList.add("product-card", "border", "rounded-3xl", "p-0", "m-2", "bg-[url(Assets/logo.png)]", "bg-no-repeat", "bg-contain", "bg-center", "border-[#a70e9233]", "shadow-lg", "shadow-[#a70e92]/50", "hover:shadow-xl");
	const productCardHTML = `
		<div class = "bg-[#00000055] backdrop-blur-sm h-full w-full p-8 m-0 rounded-3xl">
			<img src = "Assets/${product.section}/${product.id}/${product.images[0]}" alt = "${product.name}" class = "w-full object-contain rounded-md">
			<h3 class="mt-4 text-lg font-bold text-center text-white tracking-wide" style="font-family:'exo 2', sans-serif">${product.name}</h3>
			<div class="flex justify-center mt-5 mb-2"><div class="w-10 h-px bg-white/30"></div></div>
			<p class="text-center tracking-[0.3em] text-white/60 uppercase mb-4">${product.price}</p>
...
function showProductDetails(product) {
	...
	for (let i = 0; i < product.images.length; i++) {
		...
		slideDiv.classList.add('slides', 'text-center', 'justify-center', 'h-full');
	...
	if (section === "laptops") {
		...
		productsInfo.innerHTML = `
			<h2 class="text-2xl text-mypink font-bold">${product.name}</h2>
			<p class = "text-[#FFFFFF90]"><strong class = "text-[#FFFFFF]">Price:</strong> ${product.price}</p>
			<p class = "text-[#FFFFFF90]"><strong class = "text-[#FFFFFF]">Display:</strong> ${product.display}</p>
			<p class = "text-[#FFFFFF90]"><strong class = "text-[#FFFFFF]">Processor:</strong> ${product.processor}</p>
			<p class = "text-[#FFFFFF90]"><strong class = "text-[#FFFFFF]">RAM:</strong> ${product.RAM}</p>
			<p class = "text-[#FFFFFF90]"><strong class = "text-[#FFFFFF]">Storage:</strong> ${product.storage}</p>
			<p class = "text-[#FFFFFF90]"><strong class = "text-[#FFFFFF]">Battery Life:</strong> ${product.battery}</p>
			<p class = "text-[#FFFFFF90]"><strong class = "text-[#FFFFFF]">Features:</strong> ${product.features}</p>
			<p class = "text-[#FFFFFF90]"><strong class = "text-[#FFFFFF]">Colors:</strong> ${product.colors}</p>
		`
